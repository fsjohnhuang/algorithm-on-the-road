...
思路：将边按权重从小到大排序，最多执行E轮操作，每轮操作均将权重最小且不构成环路的边加入到子集T中．当子集T的边数量达到V-1时，已经成功构成生成树，退出运算．<br>
`s02_prim`, Prim算法也称为PJD算法．<br>
思路：与dijkstra算法类似，同样分为＂确定集＂和＂估计集＂两个集合，但不同的是两个集合表示的不是相对某顶点到其他顶点的距离，而是最小生成树到各顶点的距离，与此每轮＂松弛＂操作时均要检查该边两端顶点是否已在确定中，若在则忽略，防止产生环路．
### 割点
在无向图中，如果删除某顶点，则导致图不再连同（任意两点间不能相互到达），那么该顶点称为＂割点＂．<br>
`s03_cut_vertex`，重要城市－图的割点<br>
思路：构造生成树，在构造的过程中为每个顶点标识＂时间戳＂，并在经过每个顶点时通过以该顶点为根进行深度优先遍历检查该顶点是否可以绕过父顶点到达其祖先顶点，若可以则以祖先顶点的父＂时间戳＂作为自己的父＂时间戳＂，若不可以则以父顶点的＂时间戳＂作为自己的父＂时间戳＂，若没有父顶点则以自己的＂时间戳＂作为父＂时间戳＂．若父＂时间戳＂与父顶点的父＂时间戳＂不同，那么父顶点就是＂割点＂．<br>
注意：当生成树的根节点有两个子节点时，那么根节点也是割点．<br>
### 割边（或称为 桥）
在无向图中，如果删除某边，则导致图不再连同（任意两点间不能相互到达），那么该顶点称为＂割边＂．<br>
`s04_cut_edge`，重要城市－图的割边<br>
### 二分图
对于无向图，若可将所有顶点分为两个集合，并且所有边的两端分别属于不同的集合，那么该无向图称为**二分图**．<br>
**二分图的最大匹配**，就是求最大的匹配数．<br>
求二分图的最大匹配的方法：<br>
1.蛮力法．时间复杂度为`O(N!)`<br>
进行A(n, n-1)排列，然后根据有效边作筛选．<br>
2.匈牙利算法．时间复杂度为`O(NM)`<br>
在任务调度，工作安排上有所应用．<br>
思路：<br>
1. 明确最大匹配上限为N/2;<br>
2. 从每个为配对的顶点开始，通过深度优先或广度优先找到配对顶点（即找到一条增广路），若配对失败则尝试修改可通达顶点的配对顶点，从而修改已有的配对来让出与当前操作的顶点匹配的顶点．
`s05_max_match`
